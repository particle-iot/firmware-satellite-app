...
/* Particle (msom) vendoring adaptation: force the portable C implementation.
   The Particle cloud compiler does not upload .inc assembly files, and the
   once-per-session static ECDH does not need the hand-written ARM asm. Defined
   here (before types.h autodetects) so the asm_*.inc includes are never reached. */
